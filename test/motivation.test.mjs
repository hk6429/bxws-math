...
test("導師陪練只有答對才清除連錯，連錯陪練最多立即補一次", () => {
  assert.deepEqual(
    mentorCoachingTransition({ consecutiveWrong: 3, retryUsed: false }, true),
    { consecutiveWrong: 0, retryUsed: false, insertRetry: false }
  );
  assert.deepEqual(
    mentorCoachingTransition({ consecutiveWrong: 3, retryUsed: false }, false),
    { consecutiveWrong: 4, retryUsed: true, insertRetry: true }
  );
  assert.deepEqual(
    mentorCoachingTransition({ consecutiveWrong: 4, retryUsed: true }, false),
    { consecutiveWrong: 5, retryUsed: false, insertRetry: false }
  );
});
...
test("作答流程在三連錯後插入導師陪練，且不持久化連錯計數", async () => {
  ...
  assert.match(app, /const \{ qStartAt, consecutiveWrong, mentorRetryUsed, mentorPool, \.\.\.rest \} = session/);
  assert.doesNotMatch(app, /if \(inserted\) session\.consecutiveWrong = 0/);
  assert.match(app, /mentorCoachingTransition/);
});

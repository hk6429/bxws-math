...
test("首頁提供可發現的輔助設定，倒數與連詠中斷效果遵守開關", async () => {
  ...
  assert.match(html, /<summary aria-label="輔助設定">/);
...
test("疾筆與連詠效果開啟時醒目，關閉時完全沒有閃動或殘留位移", async () => {
  const css = await readFile(new URL("../css/style.css", import.meta.url), "utf8");
  assert.match(css, /quiz-timer\.timer-hot\s*\{[^}]*timer-warning-flash/);
  assert.match(css, /@keyframes timer-warning-flash[\s\S]*background-color/);
  assert.match(css, /@keyframes combo-break-shake[\s\S]*translateX\(-10px\)[\s\S]*box-shadow/);
  assert.match(css, /data-sprint-warning="off"[^}]*animation:\s*none\s*!important[^}]*background-color:\s*transparent/);
  assert.match(css, /data-combo-break-effect="off"[^}]*animation:\s*none\s*!important[^}]*transform:\s*none\s*!important/);
});

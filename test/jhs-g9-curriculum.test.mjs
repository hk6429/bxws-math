...
const expectedByStrand = {
  algebra: [
    "quadratic-equation", "quadratic-application",
  ],
  "space-shape": [
    "similar-figures", "similar-applications",
    "circle-angle", "circle-chord-tangent",
  ],
  "relation-pattern": [
    "quadratic-function",
  ],
...
test("九年級新增 8 節點依指定 strand 完整落位", () => {
...
test("九年級新節點 schema 完整且皆已上線", () => {
  for (const id of newIds) {
    ...
    assert.equal(node.tier, "jhs-g9", `${id}.tier 應為 jhs-g9`);
...
test("九年級新節點 prereq 皆可解析且接上真實先備鏈", () => {
  for (const id of newIds) {
    for (const prereqId of nodesById[id].prereq) {
      assert.ok(nodesById[prereqId], `${id} 的 prereq ${prereqId} 不存在`);
    }
  }
  assert.deepEqual(nodesById["quadratic-equation"].prereq, ["factorization"]);
  assert.deepEqual(nodesById["quadratic-application"].prereq, ["quadratic-equation"]);
  assert.deepEqual(nodesById["similar-figures"].prereq, ["triangle-congruence"]);
  assert.deepEqual(nodesById["similar-applications"].prereq, ["similar-figures"]);
  assert.deepEqual(nodesById["circle-angle"].prereq, ["geometry-symbols"]);
  assert.deepEqual(nodesById["circle-chord-tangent"].prereq, ["circle-angle"]);
  assert.deepEqual(nodesById["quadratic-function"].prereq, ["linear-function-graph", "quadratic-equation"]);
  assert.deepEqual(nodesById["probability-computation"].prereq, ["probability-basic"]);
});

test("九年級 8 節點各有 8 挑戰 × 3 自編變式，答案索引均衡不可矇答", async () => {

...
test("center 里程碑：totalMasteredCount 達 1 / 25 / 50 分別解鎖三件中心裝飾（冷啟動首件降到 1）", () => {
  ...
  assert.deepEqual(centerUnlocked(0), []);
  assert.deepEqual(centerUnlocked(1), [1]);
  assert.deepEqual(centerUnlocked(9), [1]);
  assert.deepEqual(centerUnlocked(25), [1, 25]);
  assert.deepEqual(centerUnlocked(50), [1, 25, 50]);
});

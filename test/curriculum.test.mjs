...
const readyGates = {
  ...
  // 八年級 8 節點（Codex 產、主線程獨立驗收＋保值置換均衡答案索引）
  "multiplication-formulas": ["20-6", "20-8"],
  "polynomial-operations": ["21-6", "21-8"],
  "factorization": ["22-6", "22-8"],
  "simultaneous-linear-2var": ["23-6", "23-8"],
  "square-root-radical": ["24-6", "24-8"],
  "pythagorean-theorem": ["25-6", "25-8"],
  "triangle-congruence": ["26-6", "26-8"],
  "linear-function-graph": ["27-6", "27-8"],
  // 八年級第二批 8 節點（Codex 產、主線程獨立驗收，索引已前置均衡）
  "algebraic-fraction": ["28-6", "28-8"],
  "algebraic-fraction-ops": ["29-6", "29-8"],
  "radical-operations": ["30-6", "30-8"],
  "triangle-angle-sum": ["31-6", "31-8"],
  "parallelogram-properties": ["32-6", "32-8"],
  "special-quadrilaterals": ["33-6", "33-8"],
  "linear-function-application": ["34-6", "34-8"],
  "frequency-distribution": ["35-6", "35-8"],
  // 九年級 8 節點（Codex 產、主線程獨立驗收＋數學抽驗＋索引前置均衡）
  "quadratic-equation": ["36-6", "36-8"],
  "quadratic-application": ["37-6", "37-8"],
  "similar-figures": ["38-6", "38-8"],
  "similar-applications": ["39-6", "39-8"],
  "circle-angle": ["40-6", "40-8"],
  "circle-chord-tangent": ["41-6", "41-8"],
  "quadratic-function": ["42-6", "42-8"],
  "probability-computation": ["43-6", "43-8"],
};

test("全技能骨架共 134 節點，id 全域唯一", () => {
  assert.equal(nodes.length, 134);
  assert.equal(new Set(nodes.map((node) => node.id)).size, nodes.length);
  assert.equal(nodes.filter((node) => !existingIds.has(node.id) && !newestIds.has(node.id)).length, 110);
...
test("31 個幾何節點全數完成，七年級以下皆有 lessonMedia，無 pending 骨架", () => {
  const geometry = tree.strands.find((strand) => strand.id === "space-shape");
  assert.ok(geometry);
  assert.equal(geometry.nodes.length, 31);
  ...
  assert.equal(readyGeometry.length, 31);
  assert.equal(pendingGeometry.length, 0);
  // 八、九年級幾何走文字先行、圖後補，比照七年級那輪 SVG 補圖，暫不強制 lessonMedia
  const mediaGeometry = readyGeometry.filter((node) => node.tier !== "jhs-g8" && node.tier !== "jhs-g9");
  for (const node of mediaGeometry) {
  ...
  assert.equal(new Set(mediaGeometry.map((node) => node.lessonMedia.src)).size, 10);
});

...
test("全技能骨架共 118 節點，id 全域唯一", () => {
  assert.equal(nodes.length, 118);
  assert.equal(new Set(nodes.map((node) => node.id)).size, nodes.length);
  assert.equal(nodes.filter((node) => !existingIds.has(node.id) && !newestIds.has(node.id)).length, 94);
...
test("24 個幾何節點全數完成，七年級以下皆有 lessonMedia，無 pending 骨架", () => {
  const geometry = tree.strands.find((strand) => strand.id === "space-shape");
  assert.ok(geometry);
  assert.equal(geometry.nodes.length, 24);
  ...
  assert.equal(readyGeometry.length, 24);

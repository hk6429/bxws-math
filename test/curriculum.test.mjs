...
test("全技能骨架共 126 節點，id 全域唯一", () => {
  assert.equal(nodes.length, 126);
  assert.equal(new Set(nodes.map((node) => node.id)).size, nodes.length);
  assert.equal(nodes.filter((node) => !existingIds.has(node.id) && !newestIds.has(node.id)).length, 102);
...
test("27 個幾何節點全數完成，七年級以下皆有 lessonMedia，無 pending 骨架", () => {
  const geometry = tree.strands.find((strand) => strand.id === "space-shape");
  assert.ok(geometry);
  assert.equal(geometry.nodes.length, 27);
  ...
  assert.equal(readyGeometry.length, 27);
  assert.equal(pendingGeometry.length, 0);
  // 八年級幾何（畢氏、全等）走文字先行、圖後補，比照七年級那輪 SVG 補圖，暫不強制 lessonMedia
  const mediaGeometry = readyGeometry.filter((node) => node.tier !== "jhs-g8");
